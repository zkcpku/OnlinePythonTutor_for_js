...
// session.connect();
// session.on("newListener",listener);
// console.log(session.setListener);
// assert(false);
...
// console.log(argv);
// assert(false);
// { _: [ 'test_demo\\data-types.js' ], prettydump: 'true' }
...
// 重写assert，需要使用log，因为用到了Debug功能，所以需要重定向
...
// 在代码文件前自动加上use strict标记和debugger标记
...
// function encodeObject(o,must_use = false) {
  function encodeObject(o) {
  if (DEBUG_LOG) {log('encodeObject', o);}
  // if (must_use && !_.isObject(o) && !_.isNull(o)) {
  //   // console.warn(typeof(o));
  //   // console.warn(o);
  //   let new_o = new Object();
  //   if (!_.has(new_o, 'smallObjId_hidden_')) {
  //     // make this non-enumerable so that it doesn't show up in
  //     // console.log() or other inspector functions
  //     Object.defineProperty(new_o, 'smallObjId_hidden_', { value: smallObjId,
  //                                                      enumerable: false });
  //     smallObjId++;
  //   }
  //   assert(new_o.smallObjId_hidden_ > 0);
  //   var newEncodedObj = [];

  //   encodedHeapObjects[String(new_o.smallObjId_hidden_)] = newEncodedObj;

  //   var ret = ['REF', new_o.smallObjId_hidden_];
  ...
  if (_.isNumber(o)) {
    ...
    if (encodedHeapObjects[String(o.smallObjId_hidden_)] !== undefined) {
      ...
      // let new_o = Object.getPrototypeOf(o);
      // let new_o_rst = encodeObject(new_o,must_use = true);
      // newEncodedObj.push(['[[prototype]]', new_o_rst]);


      // new_o = o.prototype;
      // new_o_rst = encodeObject(new_o,must_use = true);
      // newEncodedObj.push(['prototype', new_o_rst]);
...
function bar(x) {
        ...
        */
        ...
        // if (_.isObject(proto) && !_.isEmpty(proto)) {
          ...
          // 增加三个重要的部分：
          // [[prototype]、prototype、constructor的关系
          newEncodedObj.push(['[[prototype]]', encodeObject(proto)]);
          var new_proto = o.prototype;
          newEncodedObj.push(['prototype', encodeObject(new_proto)]);
          var new_new_proto = o.constructor;
          newEncodedObj.push(['constructor', encodeObject(new_new_proto)]);
        // }
      }
  ...
  if (all_userscript_frames.length > 0) {
    ...
    for (i = 0;
      ...
      if (receiver.type_ === 'object') {
        ...
        // console.log(">>>>>>>>>>>>",realThis,typeof(realThis));
      ...
      */
      ...
      for (ii = f.scopeCount() - 1;
        ...
        */
        ...
        // if (scopeType === 0) {
        //   console.log("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
        //   console.log(sc.details_.details_);
        //   console.log(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
      ...
      if (scopeType === 0 || scopeType === 6) {
        ...
        for (jj = 0; jj < globalScopePairs.length; jj++) {
          ...
          if (!_.has(IGNORE_GLOBAL_VARS, globalVarname)) {
            ...
            // 修改：
            // if (globalVarname.indexOf('COUNTER_') != -1) {
            //   continue;
            // }
        ...
        for (jj = 0; jj < globalCatchScopePairs.length; jj++) {
          ...
          // 修改：
            // if (globalCatchVarname.indexOf('COUNTER_') != -1) {
            //   continue;
            // }
...
// 最终结果处理，先恢复重定向，再根据要求输出到文件或者命令行中
function finalize() {
